...
class App(CTk):
    def __init__(self):
        ...
        self.logo_label = CTkLabel(
            self.sidebar_frame, text=APP_NAME, font=CTkFont(size=20, weight="bold"))
        self.logo_label.grid(row=0, column=0, padx=20, pady=(20, 10))

        self.freelance_websites_label = CTkLabel(
            self.sidebar_frame, text=Sidebar.WEBSITE_LABEL)
        self.freelance_websites_label.grid(
            row=1, column=0, padx=20, pady=(20, 10))

        self.sidebar_button_1 = CTkButton(
            self.sidebar_frame, text=Sidebar.TASKER_LABEL, command=self.open_web(Sidebar.TASKER_URL))
        self.sidebar_button_1.grid(row=2, column=0, padx=20, pady=10)

        self.sidebar_button_2 = CTkButton(
            self.sidebar_frame, text=Sidebar.CASEPLUS_LABEL, command=self.open_web(Sidebar.CASEPLUS_URL))
        self.sidebar_button_2.grid(row=3, column=0, padx=20, pady=10)

        self.sidebar_button_3 = CTkButton(
            self.sidebar_frame, text=Sidebar.FREELANCER_LABEL, command=self.open_web(Sidebar.FREELANCER_URL))
        ...
        self.appearance_mode_optionemenu = CTkOptionMenu(self.sidebar_frame, values=Sidebar.APPEARANCE_LIST,
        ...
        for i in range(100):
            switch = CTkSwitch(master=self.scrollable_frame,
                               text=f"CTkSwitch {i}")
